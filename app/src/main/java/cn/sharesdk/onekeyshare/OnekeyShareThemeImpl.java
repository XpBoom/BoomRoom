...
/** 快捷分享的主题样式的实现父类 */
public abstract class OnekeyShareThemeImpl implements PlatformActionListener,
		Callback {
	...
	final boolean isUseClientToShare(Platform platform) {
		...
		if ("Wechat".equals(name) || "WechatMoments".equals(name)
			...
			if ("true".equals(platform.getDevinfo("ShareByAppClient"))) {
				...
				test.setType("image/*");
				ResolveInfo ri = platform.getContext().getPackageManager()
						.resolveActivity(test, 0);
				if (ri == null) {
					test = new Intent(Intent.ACTION_SEND);
					test.setPackage("com.sina.weibog3");
					test.setType("image/*");
					ri = platform.getContext().getPackageManager()
							.resolveActivity(test, 0);
				}
				return (ri != null);
			}
	...
	final boolean formateShareData(Platform plat) {
		...
		boolean isWechat = "WechatFavorite".equals(name)
				|| "Wechat".equals(name) || "WechatMoments".equals(name);
		...
		if (!shareParamsMap.containsKey("shareType")) {
			...
			if (imagePath != null && (new File(imagePath)).exists()) {
				...
				} else if (shareParamsMap.containsKey("url")
						&& !TextUtils.isEmpty(shareParamsMap.get("url")
								.toString())) {
					shareType = Platform.SHARE_WEBPAGE;
					if (shareParamsMap.containsKey("musicUrl")
							&& !TextUtils.isEmpty(shareParamsMap
									.get("musicUrl").toString()) && isWechat) {
						shareType = Platform.SHARE_MUSIC;
					}
				}
			} else {
				Bitmap viewToShare = R.forceCast(shareParamsMap
						.get("viewToShare"));
				if (viewToShare != null && !viewToShare.isRecycled()) {
					shareType = Platform.SHARE_IMAGE;
					if (shareParamsMap.containsKey("url")
							&& !TextUtils.isEmpty(shareParamsMap.get("url")
									.toString())) {
						shareType = Platform.SHARE_WEBPAGE;
						if (shareParamsMap.containsKey("musicUrl")
								&& !TextUtils.isEmpty(shareParamsMap.get(
		...
		UIHandler.sendEmptyMessage(0, new Callback() {
			public boolean handleMessage(Message msg) {
				...
				if (resId > 0) {
					...
					Toast.makeText(context, resOrName, Toast.LENGTH_SHORT)
							.show();
				}
				return false;
			}
	...
	protected abstract void showEditPage(Context context, Platform platform,
			ShareParams sp);
	...
	public final boolean handleMessage(Message msg) {
		...
		case 2: {
			...
			if ("WechatClientNotExistException".equals(expName)
				...
				toast("ssdk_whatsapp_client_inavailable");
			} else if ("FacebookMessengerClientNotExistException"
					.equals(expName)) {
				toast("ssdk_facebookmessenger_client_inavailable");
			} else {
				toast("ssdk_oks_share_failed");
