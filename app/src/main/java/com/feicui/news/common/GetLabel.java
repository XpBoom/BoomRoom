...
public class GetLabel {
	...
	Handler handler;
	List<LabelSub>labelcc=new ArrayList<LabelSub>();
	...
	class getlab extends Thread{
		@Override
		public void run() {
			labelcc=DBUtil.queryLabel(db);
			if(labelcc.size()>0){
				System.out.println(labelcc.size()+"------size");
				handler.obtainMessage(0, labelcc).sendToTarget();
			}
		...
		public void onResponse(JSONObject response) {
			...
			for (LabelGroup labelC : labelCs) {
				...
				for (LabelSub labelCC : labelccs) {
					...
					handler.obtainMessage(0, labelcc).sendToTarget();//返回获取到的数据
				}
			}
