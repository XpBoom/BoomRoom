...
public class CollectActivity extends MyBaseActivity{
	private ListView lv_collect;//�ղ�չʾListView
	private List<NewsCollect> collectionlist;
	private int width;
	...
	Handler handler =new Handler(){
		...
		public void handleMessage(android.os.Message msg) {
			...
			switch (what) {
			case 1:
				 collectionlist =(List<NewsCollect>) msg.obj;
			...
			case 2:
				...
				if(collectionlist.size()>0&&newslist.size()>0){
					for (News news : newslist) {
						
					for (NewsCollect newsCollection : collectionlist) {
						if(newsCollection.getNid()==news.getNid()&&newsCollection.getFlag()==1)
						{
							newss.add(news);
						}
					}
	...
	class PopOnClick implements OnClickListener{
		...
		@Override
		public void onClick(View arg0) {//ɾ���ղ�
